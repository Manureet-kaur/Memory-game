...
function flip_card(ev) {
  if (game_state === 1) {
    if (
      $(this).attr("card_state") === "closed" &&
      $('card[card_state="open"]').length < 2
    ) {
      ...
      setTimeout(() => {
        card.attr("src", "images/" + card_positions[this_position]);
      }, 500);
...
function match_cards() {
  ...
  if (
    card_positions[first_card_position] === card_positions[second_card_position]
  ) {
    score += 10;
